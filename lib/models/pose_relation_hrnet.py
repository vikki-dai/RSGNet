...
class KBRelationKptNet(PoseHighResolutionNet):
    ...
    def __init__(self, cfg, is_train=True, **kwargs):
        ...
        num_channels = pre_stage_channels[0] + pre_stage_channels[1] + pre_stage_channels[2] + pre_stage_channels[3]
        # self.vis_rel_layer = nn.Conv2d(
        #     in_channels=num_channels,
        #     out_channels=num_channels,
        #     kernel_size=1,
    ...
    def forward(self, x, kpt_cat_maps=None, interference_scores=None, is_train=False,gt_all_kpt_scores=None,gt_amaps=None):
        # visual embedding
        features = self._forward_backbone(x)
        # multi-instances kpt maps & visual features
        all_kpt_scores = self.final_layer(features[0])
        # all_kpt_scores = gt_all_kpt_scores
        # vis_maps = self.vis_rel_layer(features[0])
        # vis_maps = F.relu(vis_maps)
        # p5
        p5 = self.deconv_layers_p5(features[-1])

        # p4
        p4 = self.deconv_layers_p4(features[-2])

        # p3
        p3 = self.deconv_layers_p3(features[-3])

        # p2
        p2 = self.conv_layers_p2(features[0])
        # relation modeling
        # step 1 prepare candidate points
        if is_train:
            assert kpt_cat_maps is not None,'kpt onehot is none'
        ...
        # compute assocation maps
        amaps = self.association_net(p2, kpt_cat_maps)
        # amaps = gt_amaps
        with torch.no_grad():
            # if interference_scores is None:
            interference_scores = []
            up_size = all_kpt_scores.size(2)*all_kpt_scores.size(3)
            up_amaps = F.interpolate(amaps.unsqueeze(1), (up_size, up_size), mode="bilinear", align_corners=False).squeeze(1)
            for amap, hmap in zip(up_amaps, all_kpt_scores):
                amap = amap.detach().cpu().numpy()
                hmap = hmap.detach().cpu().numpy()
                interference_scores.append(_generate_interference_maps_from_predictions(amap, hmap, self.heatmap_size))
            interference_scores = torch.cat(interference_scores, dim=0).to(all_kpt_scores.device)
            calibration_map = all_kpt_scores - interference_scores
            calibration_map = F.relu(calibration_map)
                # print(interference_scores.size())
        ...
        x = torch.cat([p2, p3, p4, p5, calibration_map], 1)
        refine_feat = self.refine_net(x)
        refine_weights = self.kt_machine(self.final_layer.weight)
...
class JRMKptNet(PoseHighResolutionNet):
    ...
    def __init__(self, cfg, is_train=True, **kwargs):
        ...
        kpt_weight_size = self.final_layer.weight.size(1) * self.final_layer.weight.size(
            2) * self.final_layer.weight.size(3)
        # self.refine_net = self._make_stack_conv_layer(1, num_channels, 256, 3)
        self.kt_machine = KTMachine(cfg, kpt_weight_size)
    ...
    def forward(self, x, kpt_cat_maps=None, interference_scores=None, is_train=False,gt_all_kpt_scores=None,gt_amaps=None):
        ...
        # refine_feat = self.refine_net(x)
        refine_weights = self.kt_machine(self.final_layer.weight)
        kpt_scores = nn.functional.conv2d(features[0], weight=refine_weights, padding=0, stride=1)
        return [kpt_scores, all_kpt_scores, None]

class JRMRCKptNet(PoseHighResolutionNet):
    ...
    def __init__(self, cfg, is_train=True, **kwargs):
        ...
        # knowledge based refine modules
        self.refine_net = self._make_stack_conv_layer(4, num_channels+cfg.MODEL.NUM_JOINTS, 256, 3)
        self.target_kpt_layer = nn.Conv2d(
            in_channels=256,
            out_channels=cfg.MODEL.NUM_JOINTS,
            kernel_size=1,
            stride=1,
    ...
    def forward(self, x, kpt_cat_maps=None, interference_scores=None, is_train=False,gt_all_kpt_scores=None,gt_amaps=None):
        ...
        x = torch.cat([p2, p3, p4, p5, all_kpt_scores], 1)

        refine_feat = self.refine_net(x)
        kpt_scores = self.target_kpt_layer(refine_feat)

        return [kpt_scores, all_kpt_scores, None]

class JRMPLUSRCKptNet(PoseHighResolutionNet):

    def __init__(self, cfg, is_train=True, **kwargs):
        super().__init__(cfg, **kwargs)
        self.is_train = is_train
        self.heatmap_size = np.array(cfg.MODEL.HEATMAP_SIZE)
        # self.loc_maps = self.build_geometry_embeddings((self.heatmap_size[0], self.heatmap_size[1]))
        pre_stage_channels = self.stage4_cfg['NUM_CHANNELS']

        # self.loc_emb = nn.Sequential(
        #     nn.Conv2d(4, pre_stage_channels[0], 1, bias=False),
        #     nn.ReLU(inplace=True),
        # )
        num_channels = 1 * pre_stage_channels[0] + pre_stage_channels[1] + pre_stage_channels[2] + pre_stage_channels[3]
        self.deconv_layers_p5 = self._make_deconv_layer(
            3,
            pre_stage_channels[-1],
            [pre_stage_channels[-1],pre_stage_channels[-1],pre_stage_channels[-1]],
        ...
        # knowledge based refine modules
        self.refine_net = self._make_stack_conv_layer(4, num_channels+1, 256, 3)
        self.target_kpt_layer = nn.Conv2d(
            in_channels=256,
            out_channels=cfg.MODEL.NUM_JOINTS,
            kernel_size=1,
    ...
    def forward(self, x, kpt_cat_maps=None, interference_scores=None, is_train=False,gt_all_kpt_scores=None,gt_amaps=None):
        ...
        all_kpt_scores = self.final_layer(features[0])
        with torch.no_grad():
            # loc_maps = self.loc_maps.repeat(b, 1, 1, 1)
            attended_scores, _ = torch.max(all_kpt_scores, dim=1, keepdim=True)
            # attended_locs = attended_scores * loc_maps
        # geo_feat = self.loc_emb(attended_locs)
        # p5
        p5 = self.deconv_layers_p5(features[-1])
        ...
        x = torch.cat([p2, p3, p4, p5, attended_scores], 1)
        refine_feat = self.refine_net(x)
        kpt_scores = self.target_kpt_layer(refine_feat)
        #
        return [kpt_scores, all_kpt_scores]
...
def _generate_calibration_maps_from_predictions(amap, kpt_map, map_size, thresh=0.99):

    cali_maps_all = []
    for k, hm in enumerate(kpt_map):
        cali_maps = np.zeros((map_size[1], map_size[0]))
        locs = np.where(hm >= thresh)
        if len(locs[0]) == 0:
            cali_maps_all.append(cali_maps[None])
            continue
        scores = hm[locs]
        locs = np.asarray(locs).transpose()
        ...
        for p in target_points:
            if p[3] >= max_score:
                ...
                cali_maps = np.maximum(cali_maps, map)
        cali_maps_all.append(cali_maps[None])
        # print(inter_maps_all[-1].shape)
    inter_maps_all = torch.from_numpy(np.concatenate(cali_maps_all, axis=0)).float()
...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    model = JRMKptNet(cfg, **kwargs)
    model = JRMRCKptNet(cfg, **kwargs)
    model = JRMPLUSRCKptNet(cfg, **kwargs)
